...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        to_be_processed_queue = PriorityQueue()
        for node in nodes:
        ...
        for node in nodes:
            to_be_processed_queue.insert(node.queue_key)
        # initialisation ends

        while len(to_be_processed_queue) > 0:
            u_ndp = to_be_processed_queue.extract_min() # shortest part for u is calculated, so remove it from the queue
            u_node = u_ndp.node
            u_distance = u_ndp.distance

            if u_node is destination:   # shortest path to destination calculated?
                break

            for v_node in u_node.adj:
                # relax(u, v, w)
                v_ndp = v_node.queue_key
                v_distance = v_ndp.distance
                v_new_distance = u_distance + weight(u_node, v_node)
                
                if v_distance > v_new_distance:
                    v_ndp.distance = v_new_distance
                    v_node.parent = u_node
                    to_be_processed_queue.decrease_key(v_ndp)

        # build the path using parent pointers
        path = deque()
        current = destination
